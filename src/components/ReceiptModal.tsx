...
export default function ReceiptModal({ sale, repair, branches = [], onClose, className = '' }: ReceiptModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto" id="pos-receipt-modal">
      <div className={`bg-white text-slate-900 w-full rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative flex flex-col transition-all duration-500 overflow-hidden border border-slate-100 ${
        ...
        {isPrinting && (
          <div className="absolute inset-0 bg-emerald-500/[0.015] pointer-events-none overflow-hidden z-30">
        ...
        <div className="relative w-full bg-slate-950 text-slate-100 rounded-xl p-3 border border-slate-900 shadow-xl overflow-hidden mb-4 font-mono text-[10px] flex flex-col gap-1.5 shrink-0 select-none">
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/5 via-sky-500/5 to-emerald-500/5 opacity-40" />
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 rounded-full ${isPrinting ? 'bg-amber-500 animate-ping' : 'bg-emerald-500'} shrink-0`} />
          ...
          {/* Progress Bar */}
          <div className="w-full h-1.5 bg-slate-900 rounded-full overflow-hidden relative z-10 border border-slate-800">
            <div 
          ...
          {printFormat === 'thermal' ? (
            ...
            <div className="font-mono text-xs space-y-4">
              ...
              <div className="flex flex-col items-center justify-center space-y-1 pb-3 border-b border-dashed border-slate-300 text-center">
                <div className="bg-slate-950 text-white px-3 py-1 rounded-md border border-slate-900 flex items-center space-x-1.5 font-sans leading-none shadow-sm mb-1 select-none">
                  ...
                  <span className="text-sky-400 font-black tracking-widest text-sm">K</span>
                ...
                <p className="text-[9px] text-slate-400">Tel: {matchedBranch.phone}</p>
                <p className="text-[9px] text-slate-400">{repair ? 'Repair Diagnostic Ticket' : 'Myanmar Kyat Receipt (MMK)'}</p>
              </div>
              ...
              <div className="pt-2 flex flex-col items-center justify-center space-y-1 border-t border-dashed border-slate-300">
                <div className="h-6 w-44 bg-slate-950 flex items-center justify-center tracking-[5px] text-white text-[8px] font-sans font-black select-none">
                  ||||| | ||||| || ||| | || ||||
                </div>
                <span className="text-[8px] text-slate-400 font-sans tracking-wide">ELECTRONIC FISCAL RECORD SYNCED</span>
                <p className="text-[7px] text-slate-400 font-mono">Thank you for trusting AKK Mobile Service!</p>
              </div>
            </div>
          ) : (
            /* ==========================================
            ...
            <div className="font-sans text-xs space-y-6 p-2 text-slate-800">
              ...
              <div className="flex justify-between items-start border-b border-slate-200 pb-5">
                ...
                <div className="flex items-center space-x-3.5">
                  <div className="flex items-center justify-center bg-slate-950 text-white rounded-xl p-3.5 shadow-md border border-slate-900 shrink-0 select-none">
                    <div className="flex flex-col items-center justify-center font-mono leading-none">
                      ...
                      <span className="text-[7px] text-slate-400 font-bold uppercase tracking-widest mt-1">Mobile</span>
                    </div>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 gap-8 border-b border-slate-200 pb-5">
                <div className="space-y-1">
                  <span className="text-[9px] text-slate-400 font-bold uppercase block tracking-wider">Service Branch Location</span>
                ...
                <div className="space-y-1">
                  <span className="text-[9px] text-slate-400 font-bold uppercase block tracking-wider">Billed To Customer</span>
              ...
              <div className="grid grid-cols-4 gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100 font-mono text-[10px]">
                <div>
                  <span className="text-slate-400 block font-bold">{repair ? 'TICKET NO' : 'INVOICE NO'}</span>
                  <strong className="text-slate-950">{repair ? repair.id : (sale?.id || '')}</strong>
                </div>
                <div>
                  <span className="text-slate-400 block font-bold">{repair ? 'REPAIR STATE' : 'PAYMENT METHOD'}</span>
                  <strong className="text-slate-950 uppercase text-indigo-600">{repair ? repair.status : (sale?.paymentMethod || '')}</strong>
                </div>
                <div>
                  <span className="text-slate-400 block font-bold">{repair ? 'RESPONSIBLE TECH' : 'CASHIER TERM'}</span>
                  <strong className="text-slate-950">{repair ? (repair.assignedTechnician || 'Lead Technician') : (sale?.cashierName || '')}</strong>
                </div>
                <div>
                  <span className="text-slate-400 block font-bold">{repair ? 'WARRANTY PLAN' : 'TAX STATUS'}</span>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {repair ? (
                      <tr className="hover:bg-slate-50/50">
                        <td className="px-4 py-4 text-slate-400 font-mono">1</td>
                        ...
                        <td className="px-4 py-4">
                          ...
                          {repair.technicianNotes && (
                            <p className="text-[9px] text-slate-400 italic mt-0.5">Note: {repair.technicianNotes}</p>
                          )}
                        </td>
                      ...
                      sale?.items.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <span className="text-[9px] text-slate-400 font-mono">SKU: {item.productId}</span>
                          </td>
              ...
              {/* Signature lines */}
              <div className="pt-8 flex justify-between items-center text-center font-mono text-[9px] text-slate-400 border-t border-slate-100">
                <div className="space-y-1">
                ...
                <div>
                  <div className="h-6 w-32 bg-slate-100 border border-slate-200 mx-auto flex items-center justify-center font-sans font-black tracking-wide text-slate-400 text-[8px] select-none">
                    AKK SYNCED
                  </div>
        ...
        <div className="flex items-center justify-end space-x-2 border-t border-slate-100 pt-3 mt-3 shrink-0 font-sans">
          {isPrinting ? (
            <div className="flex items-center space-x-2 text-slate-400 font-mono text-[10px] uppercase font-bold tracking-widest mr-2 py-2">
            ...
            <>
              ...
              <button
                onClick={onClose}
                className="bg-slate-950 hover:bg-slate-850 text-white font-bold py-2 px-4 rounded-xl text-xs transition-all flex items-center justify-center space-x-1 hover:scale-[1.02] active:scale-95"
              >

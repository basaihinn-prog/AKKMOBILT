...
export default function RepairCenter({ repairs, activeBranchId, onRefresh }: RepairCenterProps) {
  ...
  const getBadgeStyle = (status: RepairStatus) => {
    switch (status) {
      case 'received': return 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20';
      case 'diagnostic': return 'bg-amber-500/10 text-amber-400 border border-amber-500/20 animate-pulse';
      case 'repairing': return 'bg-sky-500/10 text-sky-400 border border-sky-500/20';
      case 'testing': return 'bg-purple-500/10 text-purple-400 border border-purple-500/20';
      case 'ready': return 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 font-bold';
      case 'delivered': return 'bg-slate-500/10 text-slate-400 border border-slate-500/25';
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6" id="repair-center-module">
      ...
      <div className="xl:col-span-7 space-y-5">
        <div className="bg-slate-900/30 border border-slate-900 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button
            ...
            className="w-full sm:w-auto bg-indigo-500 hover:bg-indigo-400 text-slate-950 font-black text-xs px-4 py-2.5 rounded-xl transition flex items-center justify-center space-x-1.5"
          >
        ...
        {isCreating && (
          <form onSubmit={handleCreate} className="bg-slate-900/40 border border-indigo-500/20 rounded-2xl p-5 space-y-4 font-mono text-xs animate-slide-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-slate-400">Customer Phone</label>
                <input
                  type="tel"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-slate-400">Device Model</label>
                <input
                  type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-slate-400">Hardware Issue Description</label>
              <textarea
                placeholder="Front glass shattered, touch sensor flickering, water diagnostics needed..."
                value={issue}
                onChange={(e) => setIssue(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-100 outline-none h-20"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-slate-400">Warranty (Months)</label>
                <input
                  type="number"
                  value={warranty}
              ...
              <div className="flex items-end">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-indigo-500 hover:bg-indigo-400 text-slate-950 font-black py-2.5 rounded transition uppercase font-sans text-xs"
                >
        ...
        <div className="space-y-3">
          {filtered.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between gap-3 font-mono text-xs">
                  <div className="space-y-1.5 min-w-0">
                    ...
                    <p className="font-bold text-slate-200">{ticket.deviceBrand} {ticket.deviceModel}</p>
                    <p className="text-[11px] text-slate-400 truncate max-w-[320px]">{ticket.issueDescription}</p>
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-slate-500 pt-1 border-t border-slate-900/50">
                  ...
                  <div className="text-right shrink-0 space-y-1">
                    <span className="text-xs font-bold text-emerald-400 block">{ticket.estimatedCost.toLocaleString()} MMK</span>
            ...
            <div className="py-12 bg-slate-900/10 border border-slate-900 rounded-2xl text-center text-slate-600 font-mono space-y-2">
      ...
      <div className="xl:col-span-5">
        {selectedTicket ? (
          <div className="bg-slate-900/20 border border-slate-900 rounded-2xl p-5 space-y-5 font-mono text-xs" id="technician-workspace">
            <div className="border-b border-slate-900 pb-3">
              <span className="text-[10px] text-indigo-400 font-black uppercase tracking-wider block mb-1">Active Tech Diagnostic Bench</span>
              <h4 className="font-black text-sm text-slate-100">{selectedTicket.id} - {selectedTicket.deviceModel}</h4>
            </div>

            <div className="space-y-3.5 bg-slate-950 border border-slate-900 p-4 rounded-xl text-[11px]">
              <div className="flex justify-between">
                <span className="text-slate-500">Device / Brand:</span>
                <strong className="text-slate-200">{selectedTicket.deviceBrand}</strong>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <strong className="text-slate-200">{selectedTicket.customerPhone}</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Warranty Policy:</span>
                <strong className="text-slate-200">{selectedTicket.warrantyMonths} Months</strong>
              </div>
              <div className="border-t border-slate-900 pt-2.5">
                <span className="text-slate-500 block mb-1">Issue Reported:</span>
                <p className="text-slate-300 bg-slate-900/20 p-2.5 rounded border border-slate-900 leading-relaxed text-[11px]">
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-slate-400">Diagnosis State</label>
                  <select
                    value={statusVal}
                    onChange={(e) => setStatusVal(e.target.value as RepairStatus)}
                    className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-2 text-slate-200 font-bold"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-slate-400">Billable Quote (MMK)</label>
                  <input
                    type="number"
                    value={costVal}
              ...
              <div className="space-y-1.5">
                <label className="text-slate-400">Technician Bench Notes</label>
                <textarea
                  value={techNotes}
                  onChange={(e) => setTechNotes(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 h-24 leading-relaxed"
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="submit"
                  className="w-full bg-slate-850 hover:bg-slate-800 border border-slate-750 text-slate-200 font-bold py-3 rounded-xl transition uppercase tracking-wider text-[11px]"
                >
                  Save Logs
                </button>
                <button
                  ...
                  className="w-full bg-indigo-500 hover:bg-indigo-400 text-slate-950 font-black py-3 rounded-xl transition uppercase tracking-wider text-[11px] flex items-center justify-center space-x-1.5"
                >
          ...
          <div className="bg-slate-900/10 border border-slate-900 rounded-2xl p-8 text-center text-slate-600 font-mono space-y-2 py-24">
